        ...
        stage('Push to ECR') {
            steps {
                script{
                    ...
                    docker.withRegistry('https://335871625378.dkr.ecr.eu-west-2.amazonaws.com/netflix-job', 'ecr:us-east-1:Chinwe-ECR') {
                    // build image
                    def myImage = docker.build("335871625378.dkr.ecr.us-east-1.amazonaws.com/netflix-job:latest")
                    // push image
                    myImage.push()
                    }
                }
